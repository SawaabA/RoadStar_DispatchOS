...
export function validateTelemetryEvent(value: unknown, options: { knownTruckIds: Set<string>; previousRecordedAt?: number; now?: number }): { event?: TelemetryEvent; recordedAt?: number; reason?: string } {
  ...
  if (incident != null && (
    typeof incident !== 'object' || typeof incident.id !== 'string' ||
    incident.truckId !== event.truckId || typeof incident.label !== 'string' ||
    typeof incident.detail !== 'string' || !Number.isFinite(Date.parse(incident.startedAt)) ||
    !['traffic', 'closure', 'dock_wait', 'duty_change'].includes(incident.kind) ||
    !['critical', 'warning', 'info'].includes(incident.severity)
  )) return { reason: 'invalid simulator incident' };
