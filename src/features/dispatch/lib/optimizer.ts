import type { Assignment, Coordinates, DispatchCandidate, DispatchLoad, Driver, FeasibilityReason, PlanProposal, TrailerAsset, TruckAsset } from '../types';
...
type Context = { now?: number; trucks?: TruckAsset[]; assignments?: Assignment[] };
...
export function evaluateCandidate(load: DispatchLoad, driver: Driver, trailer: TrailerAsset, context: Context = {}): DispatchCandidate {
  ...
  if (load.equipment !== trailer.type || (load.temperatureControlled && trailer.type !== 'Reefer')) reject('equipment', 'Trailer does not meet the load equipment or refrigeration requirement.');
  ...
  const score = feasible ? Math.round(Math.max(0, Math.min(100, 100 - deadheadKm / 5 + Math.min(hosRemainingAfter, 5) - 5))) : 0;
  return {
    loadId: load.id, driverId: driver.id, truckId: driver.truckId, trailerId: trailer.id,
    feasible, reasons, deadheadKm: round(deadheadKm), tripKm: round(tripKm),
...
/** Maximize covered loads, then priority, then minimize deadhead. No input mutation.
 ...
 */
export function buildMorningPlan(loads: DispatchLoad[], drivers: Driver[], trailers: TrailerAsset[], context: Context = {}): PlanProposal {
  const now = context.now ?? Date.now();
  ...
  const rows = loads.filter(l => l.status === 'unassigned').map(load => {
    const evaluated = drivers.flatMap(driver => {
      ...
      return trailer ? [evaluateCandidate(load, driver, trailer, options)] : [];
    });
  ...
  let best: DispatchCandidate[] = [], bestPriority = -1, bestKm = Infinity, nodes = 0;
  ...
  function search(index: number, points: number, km: number) {
    if (++nodes > PLANNING_ASSUMPTIONS.maxSearchNodes) return;
    if (chosen.length > best.length || (chosen.length === best.length && (points > bestPriority || (points === bestPriority && km < bestKm)))) {
      best = [...chosen]; bestPriority = points; bestKm = km;
    }
    ...
    for (const c of row.candidates) {
      ...
      search(index + 1, points + priority[row.load.priority], km + c.deadheadKm);
