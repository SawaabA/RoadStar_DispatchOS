...
- The morning planner maximizes the number of covered loads, then summed priority (critical 3, high 2, standard 1), then maximizes the weighted deadhead/on-time/HOS/future-position score, with deadhead as the final tie-breaker. It reserves each driver, truck, trailer, and load at most once, including existing active assignments supplied by the application. A deterministic 50,000-node search budget returns the best proposal found; larger fleets are not guaranteed globally optimal. Driver availability must be `Now` or a dated timestamp. No rest-related clock replenishment is inferred.
